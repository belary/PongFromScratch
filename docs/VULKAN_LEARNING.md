...
## C++ 基础：Struct vs Union

在深入 Vulkan 之前，先理解 C++ 中 **struct** 和 **union** 的区别，这对理解 Vulkan 的数据结构很重要。
...
| **成员访问** | 可以同时访问所有成员 | 同时只能访问一个成员 |
| **修改影响** | 修改一个成员不影响其他 | 修改成员会影响共享内存的所有成员 |
| **适用场景** | 需要同时使用多个字段 | 只会使用其中一个字段 |
| **示例** | `struct { int x, y; }` | `union { int a; float b; }` |
...
int main() {
    std::cout << "Struct: " << sizeof(StructExample) << "\n";   // 16
    std::cout << "Union: " << sizeof(UnionExample) << "\n";    // 8
    return 0;
}
```
...
### 本项目应用：DescriptorInfo 结构体

在 [vk_types_cakezz.h:29](../src/renderer/vk_types_cakezz.h#L29) 中：
...
struct DescriptorInfo
{
    union
    {
        VkDescriptorBufferInfo bufferInfo;  // 16 字节
        VkDescriptorImageInfo imageInfo;    // 24 字节
    };
    // ↑ union 意味着 bufferInfo 和 imageInfo 共享同一块内存
    // 总大小：24 字节（取较大的那个）
};
```
...
```cpp
// 描述符要么是缓冲区，要么是图像，不会同时是两者

// 场景 1：创建缓冲区描述符
DescriptorInfo desc1(globalUBO);
// desc1.bufferInfo 被初始化
// desc1.imageInfo 未使用（不用关心其值）

// 场景 2：创建图像描述符
DescriptorInfo desc2(sampler, imageView);
// desc2.imageInfo 被初始化
// desc2.bufferInfo 未使用（不用关心其值）
```
...
struct DescriptorInfo {
    union {
        VkDescriptorBufferInfo bufferInfo;  // 要么用这个
        VkDescriptorImageInfo imageInfo;    // 要么用这个
    };
};
// 一眼就能看出：bufferInfo 和 imageInfo 不会同时使用
```
...
```
🏠 房子 A (bufferInfo)     🏠 房子 B (imageInfo)
   16 平方米                  24 平方米
...
🏠 可变形房子
   ...
   可以变成 16 平米的办公室（bufferInfo）
   或者保持 24 平米的住宅（imageInfo）
...
// 创建缓冲区描述符
DescriptorInfo desc1(globalUBO);
// desc1.bufferInfo 有效，desc1.imageInfo 未使用

// 创建图像描述符
DescriptorInfo desc2(sampler, imageView);
// desc2.imageInfo 有效，desc1.bufferInfo 未使用
...
write.pBufferInfo = &desc1.bufferInfo;  // ← 使用 bufferInfo

// 图像描述符
write.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
write.pImageInfo = &desc2.imageInfo;    // ← 使用 imageInfo
```
...
|------|------|
| **Struct** | 所有成员独立，同时有效 |
| **Union** | 所有成员共享内存，同时只有一个有效 |
| **内存占用** | Union 取最大成员的大小 |
| **本项目应用** | DescriptorInfo 使用 union 节省内存 |
| **使用场景** | 当多个字段互斥时（只会用其中一个） |
| **注意事项** | 访问 union 成员时要确保它是当前有效的 |
...
在 Vulkan 中，**描述符类型**（Descriptor Type）定义了着色器可以访问的资源类型。不同的描述符类型有不同的性能特性和使用场景。

---

#### 本项目使用的三种描述符类型

在 [vk_renderer_cakezz.cpp:334](../src/renderer/vk_renderer_cakezz.cpp#L334) 中，定义了三种描述符集绑定：

```cpp
VkDescriptorSetLayoutBinding layoutBindings[] = {
    // [0] 顶点着色器 - 全局 Uniform Buffer
    layout_binding(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT, 1, 0),
    
    // [1] 顶点着色器 - 变换 Storage Buffer  
...
**类比**：
```
Uniform Buffer = 广播通知栏
所有实例都能看到相同的内容（屏幕尺寸）
```

---

##### 2. STORAGE_BUFFER（存储缓冲区）

**定义**：存储大量、可变长、只读或读写的数据数组
...
**本项目使用**：
```cpp
// vk_renderer_cakezz.cpp:572
vkcontext->transformStorageBuffer = vk_allocate_buffer(
    ...,
    sizeof(Transform) * MAX_ENTITIES,  // 可容纳多个实体
...
```
Storage Buffer = 大型仓库
每个实例去仓库找自己的专属位置（通过索引）
```

---

##### 3. COMBINED_IMAGE_SAMPLER（组合图像采样器）
...
| 特性 | 说明 |
|------|------|
| **组成部分** | ImageView + Sampler |
| **访问模式** | 只读 |
| **功能** | 纹理采样 + 过滤 |
| **着色器访问** | 使用 `texture()` 函数采样 |
| **硬件优化** | 专用纹理采样单元 |
...
imgInfo.imageView = textureImageView;    // 纹理视图
imgInfo.sampler = textureSampler;         // 采样器
...
**类比**：
```
Combined Image Sampler = 相册 + 放大镜
相册(Image)存储图片，放大镜(Sampler)定义如何查看
```
...
| 特性 | UNIFORM_BUFFER | STORAGE_BUFFER | COMBINED_IMAGE_SAMPLER |
|------|----------------|----------------|------------------------|
| **主要用途** | 小型全局数据 | 大型数组数据 | 纹理采样 |
| **大小限制** | ~16KB | 很大（MB 级） | 取决于纹理大小 |
| **访问方式** | 直接访问 | 数组索引访问 | `texture()` 函数 |
| **读写权限** | 只读 | 只读或读写 | 只读 |
| **更新频率** | 较少 | 可频繁 | 纹理切换时 |
| **性能** | 最快 | 较快 | 专用硬件优化 |
| **着色器阶段** | 所有阶段 | 所有阶段 | 顶点/片段/几何 |
| **本项目对应** | 屏幕尺寸 | 实体变换数组 | 精灵纹理 |
...
```
GPU 对不同类型的数据有不同的优化策略：

Uniform Buffer    → 放在快速缓存中，全芯片共享
Storage Buffer    → 放在显存中，按需读取
Image Sampler     → 专用的纹理采样单元（TMU）
```

##### 2. **硬件限制**

```
某些硬件对 Uniform Buffer 的大小有严格限制（16KB）
如果数据量大，必须使用 Storage Buffer
```
...
```glsl
// 看到声明就知道数据特点
uniform GlobalData { ... };      // 小型、只读、共享
buffer TransformData { ... };    // 大型、可索引、可读写
sampler2D texture;               // 纹理采样
```
...
Vulkan 还支持其他描述符类型，本项目未使用：

| 类型 | 用途 |
|------|------|
| `SAMPLER` | 单独的采样器 |
| `SAMPLED_IMAGE` | 单独的纹理图像 |
| `UNIFORM_TEXEL_BUFFER` | 纹理缓冲区（只读） |
| `STORAGE_TEXEL_BUFFER` | 纹理缓冲区（读写） |
| `UNIFORM_BUFFER_DYNAMIC` | 动态偏移的 Uniform Buffer |
| `STORAGE_BUFFER_DYNAMIC` | 动态偏移的 Storage Buffer |
| `INPUT_ATTACHMENT` | 帧缓冲区读取（用于后处理） |
| `STORAGE_IMAGE` | 可读写的图像（计算着色器） |
...
| 要点 | 说明 |
|------|------|
| **UNIFORM_BUFFER** | 小型全局数据（< 16KB），所有实例共享 |
| **STORAGE_BUFFER** | 大型数组数据，支持索引访问 |
| **COMBINED_IMAGE_SAMPLER** | 纹理图像 + 采样器组合 |
| **选择依据** | 数据大小、访问模式、更新频率 |
| **性能考虑** | Uniform 最快，Storage 次之，Image 有专用硬件 |
