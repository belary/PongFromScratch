...
T1 = 三角形 1 (顶点 0, 1, 2) - 顺时针
T2 = 三角形 2 (顶点 3, 4, 5) - 顺时针
```

**顶点数据表：**

| 索引 | 位置 (xy) | 纹理坐标 (zw) | 三角形 |
|------|-----------|---------------|--------|
| 0 | `(-0.5, 0.5)` | `(0.0, 1.0)` | T1 |
| 1 | `(-0.5, -0.5)` | `(0.0, 0.0)` | T1 |
| 2 | `(0.5, -0.5)` | `(1.0, 0.0)` | T1 |
| 3 | `(0.5, -0.5)` | `(1.0, 0.0)` | T2（复用） |
| 4 | `(0.5, 0.5)` | `(1.0, 1.0)` | T2 |
| 5 | `(-0.5, 0.5)` | `(0.0, 1.0)` | T2（复用） |
...
- ✅ 在 VULKAN_LEARNING.md 的"着色器"章节添加了"顶点着色器：带纹理坐标的四边形渲染"小节
- ✅ 包含：顶点数据编码技巧、几何布局图解、顶点数据表、三角形绕序验证、uv 插值机制

#### 10. 代码更新

- ✅ 更新了 [shader.vert](../assets/shaders/shader.vert) 文件，添加详细的中文注释
- ✅ 每个顶点都有清晰的注释说明其位置和纹理坐标
- ✅ 主函数的每行代码都有详细解释
...
- ✅ 三角形绘制
- ✅ 纹理加载流程（DDS → Staging Buffer → Image）
- ✅ 纹理渲染设置（DescriptorSetLayout → ImageView → Sampler → DescriptorSet）
- ✅ 顶点着色器纹理坐标传递（vec4 编码方式）

**下一步：**
- 在 Fragment Shader 中添加纹理采样代码
- 测试纹理映射效果
- 调整纹理过滤模式（NEAREST vs LINEAR）

---

## 2025-04-27 - 修正：顶点位置和坐标系统理解
...
**问题发现：**
- 用户指出：`(-0.5, 0.5)` 在 Vulkan 坐标系中应该是**左下角**，不是左上角
- 原因：Vulkan 的 Y 轴**向下为正**

**修正后的顶点位置：**

| 索引 | 坐标 | 正确位置 | 之前错误 |
|------|------|----------|----------|
| 0 | `(-0.5, 0.5)` | 左下 (y=0.5, 向下) | ✗ 左上 |
| 1 | `(-0.5, -0.5)` | 左上 (y=-0.5, 向上) | ✗ 左下 |
| 2 | `(0.5, -0.5)` | 右上 (y=-0.5, 向上) | ✗ 右下 |
| 4 | `(0.5, 0.5)` | 右下 (y=0.5, 向下) | ✗ 右上 |

#### 2. 裁剪坐标 vs NDC 坐标理解

**关键概念：**

`gl_Position` 输出的是**裁剪坐标**，不是 NDC 坐标！
...
// GPU 透视除法
ndc = vec3(xy, 0.0) / 1.0 = vec3(xy, 0.0);
//                           ↑
//                           数值相等，但概念不同！
```

**关键理解：**
- 即使 `w = 1.0` 时数值相等，`gl_Position` **仍然是裁剪坐标**
- GPU 会自动执行透视除法
- 这是图形管线的固定功能阶段
...
- ✅ 在 VULKAN_LEARNING.md 的"顶点着色器"章节添加了"坐标变换流程"小节
- ✅ 包含：完整的图形管线坐标变换流程图、裁剪坐标 vs NDC 坐标对比表、透视除法详解
- ✅ 修正了顶点数据表中的位置说明
- ✅ 修正了几何布局图解
...
- ✅ 修正了所有顶点的位置注释
- ✅ 添加了关于裁剪坐标和透视除法的详细解释
- ✅ 修正了主函数中的注释，明确说明 gl_Position 是裁剪坐标
...
**注意：** 当前顶点顺序是**逆时针**，如果 Pipeline 设置了 `frontFace = CLOCKWISE`，这些三角形会被判定为**背面**而剔除。

### 核心要点总结

1. **Vulkan Y 轴向下**：y=0.5 是下边，y=-0.5 是上边
2. **gl_Position 是裁剪坐标**：不是 NDC 坐标，GPU 会自动执行透视除法
3. **w = 1 时数值相等但概念不同**：仍然要理解这是两个不同的坐标系
4. **透视除法是自动的**：开发者只需设置 gl_Position，GPU 会处理剩下的转换
